...
RES_DATA_PATH = r'Cross_result.xlsx'
ROW_MAE, ROW_MMRE, ROW_PRED = 30, 31, 32
...
with open('source_target_pair.txt', 'r') as f:
    content = f.read()
...
def prepare_dataframe(file_name):
    ...
    order=[ 'contributors','creater_ar_count','developer_ar_count','tester_ar_count','committer_qualification_level', 'detail_version_name','author_commit_num','author_modify_file_num','author_create_mr_num','author_update_mr_num','author_mr_commit_num','dev_dts_num','test_dts_num','actual_effort',
          ]
    data=data[order]
    data = data.fillna(0)
...
def cross_project_split(train_data, test_data):
    ...
    train_ex = train_data.iloc[:train_val_split_point, 0:13]
    train_labels = (train_data['actual_effort'][:train_val_split_point])

    val_ex = train_data.iloc[train_val_split_point:, 0:13]
    val_labels = (train_data['actual_effort'][train_val_split_point:])

    test_ex = test_data.iloc[:,0:13]
    test_labels = (test_data['actual_effort'][:])
